...
| Order lines carry their **own unit price** | — | data | The ticket then survives its item being deleted mid-order, and a price edit is an event that happens *to* the line rather than a lookup that silently returns something else. The money **does not move**: a quoted price must not change under an open order, so the `New price` tag is the whole of what the cashier is told, and it clears when they ring the item up again at the new one |
